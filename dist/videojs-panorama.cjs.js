...
var BaseCanvas = function BaseCanvas(baseComponent, THREE) {
    ...
    //holding the script for camera direction 
    var MPScript;
    ...
    return {
        ...
        attachControlEvents: function attachControlEvents() {
            ...
            //attache the MPScript function to the MPScript event
            this.on('MPScript', this.handleMPScript.bind(this));
        ...
        handleMouseUp: function handleMouseUp(event) {
            ...
            if (this.clickToToggle) {
                ...
                //printing to console the current time, lat, lon and zoom to use as a direction
                console.log("'" + Math.round(this.player().currentTime()) + "' : {'zoom' : " + this.camera.fov + ", 'lat' : " + Math.round(this.lat) + ",'lon' : " + Math.round(this.lon) + "},");
            }
        },
         ...
         * 
         */
        handleMPScript: function handleMPScript(event) {
            console.log("this.isUserInteracting: " + this.isUserInteracting);
            //if(this.isUserPinch){
            var curentTime = Math.round(this.player().currentTime());
            //first the function checks if the video is running if not it set thee camare back to center
            if (this.lastTimeRead == this.player().currentTime()) {
                this.settings.initLat = this.settings.originalInitLat;
                this.settings.initLon = this.settings.originalInitLon;
            } else if (this.MPScript[curentTime] != undefined && !this.isUserInteracting) {

                //setting the max and lat and lon according to the current zoom
                this.setMaxLatAndLon(this.MPScript[curentTime].zoom);
                //setting the current lat and lon 
                this.settings.initLat = this.MPScript[curentTime].lat;
                this.settings.initLon = this.MPScript[curentTime].lon;
                //test if zoom has change
                if (this.camera.fov != this.MPScript[curentTime].zoom) {
                    setTimeout(function () {
                        this.fixZoom(this.MPScript[curentTime].zoom);
                    }.bind(this), 100);
                }
            }
            //}
            this.lastTimeRead = this.player().currentTime();
            //calling agin after 1 sec for more direction
            setTimeout(function () {
                this.handleMPScript();
            }.bind(this), 1000);
        },
        /**
         * fixing to the current zoom with animation
         */
        fixZoom: function fixZoom(zoom) {
            if (this.camera.fov > zoom) this.camera.fov -= 1;
            if (this.camera.fov < zoom) this.camera.fov += 1;
            this.camera.updateProjectionMatrix();
            this.setMaxLatAndLon(zoom);
            if (this.camera.fov != zoom) {
                setTimeout(function () {
                    this.fixZoom(zoom);
                }.bind(this), 10);
            }
        ...
        //setting the max lat and lon base on the zoom

        setMaxLatAndLon: function setMaxLatAndLon(zoom) {
            this.settings.maxLon = this.settings.originalMaxLon - 4 * ((zoom - this.settings.minFov) / 6);
            this.settings.minLon = this.settings.originalMinLon + 4 * ((zoom - this.settings.minFov) / 6);
            this.settings.minLat = this.settings.originalMinLat + 3 * ((zoom - this.settings.minFov) / 6);
            this.settings.maxLat = this.settings.originalMaxLat - 3 * ((zoom - this.settings.minFov) / 6);
        },
        ...
        handleTouchMove: function handleTouchMove(event) {
            ...
            this.settings.initLat = this.lat;
            this.settings.initLon = this.lon;
        },
...
var Canvas = function Canvas(baseComponent, THREE) {
    ...
    return util.extend(parent, {
        constructor: function init(player, options) {
            ...
            //using CylinderGeometry 
            if (this.videoType === "panorama") {
                geometry = new THREE.CylinderGeometry(10, 10, 20, 12, 1, true, 0, Math.PI);
            } else if (this.videoType === "panoramaBuffer") {
                geometry = new THREE.CylinderBufferGeometry(10, 10, 20, 12, 1, true, 0, Math.PI);
            } else if (this.videoType === "fisheye") {
        ...
        handleMouseWheel: function handleMouseWheel(event) {
            ...
            //setting MaxLatAndLon after zoom changed.
            this.setMaxLatAndLon(this.camera.fov);

            //console.log("this.camera.fov : " + this.camera.fov + " - " + this.settings.minLat);
            this.camera.updateProjectionMatrix();
            if (this.VRMode) {
        ...
        //the setting function for MaxLatAndLon after zoom changed.
        setMaxLatAndLon: function setMaxLatAndLon(zoom) {
            this.settings.maxLon = this.settings.originalMaxLon - 4 * ((zoom - this.settings.minFov) / 6);
            this.settings.minLon = this.settings.originalMinLon + 4 * ((zoom - this.settings.minFov) / 6);
            this.settings.minLat = this.settings.originalMinLat + 3 * ((zoom - this.settings.minFov) / 6);
...
var defaults$2 = {
    ...
    returnStepLat: 0.5,
    returnStepLon: 2,
    //mast be always true so direction will work
    backToVerticalCenter: true, //!runOnMobile,
    backToHorizonCenter: true, //!runOnMobile,
    clickToToggle: false,
    ...
    //mast be always false so direction will work
 ...
 */
var onPlayerReady$1 = function onPlayerReady(player, options, settings) {
...
var plugin$1 = function plugin() {
     ...
     */
    var videoTypes = ["equirectangular", "panorama", "panoramaBuffer", "fisheye", "3dVideo", "dual_fisheye"];
    var panorama = function panorama(options) {
        var _this = this;

        if (settings.mergeOption) options = settings.mergeOption(defaults$2, options);
        if (typeof settings._init === "undefined" || typeof settings._init !== "function") {
            console.error("plugin must implement init function().");
            return;
        }
        if (videoTypes.indexOf(options.videoType) == -1) options.videoType = defaults$2.videoType;
        settings._init(options);
        /* implement callback function when videojs is ready */
        this.ready(function () {
            onPlayerReady$1(_this, options, settings);
        });
    };
...
var MPScript = function MPScript(player, cameraDirection) {
  setTimeout(function (cameraDirection) {

    console.log("Script Start");
    var canvas = player.getChild('Canvas');
    if (canvas) {
      console.log("Script Exsit");
      canvas.setMPScript(cameraDirection);
      canvas.handleMPScript();
    }
  }.bind(null, cameraDirection), 1000);
};
...
var onPlayerReady = function onPlayerReady(player, options) {
  ...
  player.addClass('vjs-MP-panorama');
  ...
  ///check to see what iPhone version
  function iOSversion() {
  ...
  //test to see if user using iphone
  // and changing the source accordingly
  if (isRealIphone()) {
    var isoVar = iOSversion();

    if (isoVar[0] <= 5) {
      options.stream = options.streamOldIPhone;
    } else {
      options.stream = options.streamNewIPhone;
    }
  }
  ...
  player.panorama({
    ...
    clickAndDrag: true,
    //autoMobileOrientation mast be false so the direction will take place
    autoMobileOrientation: false,
    VREnable: isMobile(),
    NoticeMessage: isMobile() ? "please drag and drop the video" : "",
    //limit viewable area according to the current zoom
    originalMinLat: -38,
    originalMaxLat: 38,
    videoType: "panorama",
    originalMinLon: 108,
 ...
 *           An object of options left to the plugin author to define.
 */
var MPPanorama = function MPPanorama(options) {
  var _this = this;
...
// Register the plugin with video.js.
registerPlugin('MPPanorama', MPPanorama);
